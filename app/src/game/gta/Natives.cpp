...
namespace gta
{
    ...
    uint64_t Natives::GetHashByHandlerImpl(uint64_t handler)
    {
    ...
    std::unordered_map<uint64_t, uint64_t> Natives::GetAllImpl()
    {
        std::unordered_map<uint64_t, uint64_t> result;

        if (!scrDbg::g_Pointers.NativeRegistrationTable)
            return result;
