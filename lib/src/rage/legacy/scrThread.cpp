...
namespace rage::legacy
{
    ...
    scrThread* scrThread::GetThread(std::uint32_t hash)
    {
        static bool init = [] {
            if (auto addr = Memory::ScanPattern("45 33 F6 8B E9 85 C9 B8"))
                m_Threads = addr->Sub(4).Rip().Sub(8).As<shared::atArray<scrThread*>*>();
    ...
    {
        auto thread = scrThread::GetCurrentThread();
        if (!thread)
        ...
        MessageBoxA(nullptr, fullMessage, "Exception", MB_OK | MB_ICONERROR);
