...
namespace scrDbgApp
{
    ...
    private:
        ...
        static inline constexpr std::array<InstructionInfo, 131> m_InstructionTable = {{
            ...
            {"NATIVE", "Calls a native command using argument and return counts from the next byte, and a hash index from the next two bytes.", "h"},
            {"ENTER", "Marks the start of a function; defines argument count, local variable count, and name size to skip.", "n"},
            ...
            {"J", "Performs an unconditional relative jump by the signed short following the opcode.", "g"},
            {"JZ", "Performs a relative jump if the top stack value is zero; offset defined by the following short.", "g"},
            {"IEQ_JZ", "Performs a relative jump if the top two integers are not equal; offset defined by the following short.", "g"},
            {"INE_JZ", "Performs a relative jump if the top two integers are equal; offset defined by the following short.", "g"},
            {"IGT_JZ", "Performs a relative jump if the first integer is less than or equal to the second; offset defined by the following short.", "g"},
            {"IGE_JZ", "Performs a relative jump if the first integer is less than the second; offset defined by the following short.", "g"},
            {"ILT_JZ", "Performs a relative jump if the first integer is greater than or equal to the second; offset defined by the following short.", "g"},
            {"ILE_JZ", "Performs a relative jump if the first integer is greater than the second; offset defined by the following short.", "g"},
            ...
            {"SWITCH", "Performs a switch jump based on a comparison value; table and offsets follow the opcode.", "i"},
            {"STRING", "Pushes a pointer to a string from the string table using the index at the top of the stack.", "m"},
