#include "ScriptThreadGTA5_GEN8.hpp"

namespace scrDbgApp
{
    uint32_t ScriptThreadGTA5_GEN8::GetId() const
    {
        return m_Base.Add(ID).Get<uint32_t>();
    }

    uint32_t ScriptThreadGTA5_GEN8::GetProgramHash() const
    {
        return m_Base.Add(PROGRAM_HASH).Get<uint32_t>();
    }

    ScriptThread::State ScriptThreadGTA5_GEN8::GetState() const
    {
        return m_Base.Add(STATE).Get<ScriptThread::State>();
    }

    void ScriptThreadGTA5_GEN8::SetState(ScriptThread::State state) const
    {
        return m_Base.Add(STATE).Set<ScriptThread::State>(state);
    }

    uint32_t ScriptThreadGTA5_GEN8::GetPc() const
    {
        return m_Base.Add(PC).Get<uint32_t>();
    }

    uint32_t ScriptThreadGTA5_GEN8::GetFp() const
    {
        return m_Base.Add(FP).Get<uint32_t>();
    }

    uint32_t ScriptThreadGTA5_GEN8::GetSp() const
    {
        return m_Base.Add(SP).Get<uint32_t>();
    }

    uint32_t ScriptThreadGTA5_GEN8::GetStackSize() const
    {
        return m_Base.Add(STACK_SIZE).Get<uint32_t>();
    }

    ScriptThread::Priority ScriptThreadGTA5_GEN8::GetPriority() const
    {
        return m_Base.Add(PRIORITY).Get<ScriptThread::Priority>();
    }

    uint8_t ScriptThreadGTA5_GEN8::GetCallDepth() const
    {
        return m_Base.Add(CALL_DEPTH).Get<uint8_t>();
    }

    uint32_t ScriptThreadGTA5_GEN8::GetCallStack(uint32_t index) const
    {
    ...
    std::string ScriptThreadGTA5_GEN8::GetScriptName() const
    {
        char buffer[64];
