#include "DisassemblerRDR3.hpp"
#include "opcodes/OpcodesRDR3.hpp"

namespace scrDbgApp
{
    std::string DisassemblerRDR3::GetFunctionName(uint32_t pc, uint32_t size) const
    {
        if (size > 0)
        {
    ...
    std::optional<uint32_t> DisassemblerRDR3::GetStringIndex(uint32_t pc) const
    {
        OpcodesRDR3 op = static_cast<OpcodesRDR3>(GetU8(pc));

        switch (op)
        {
        case OpcodesRDR3::PUSH_CONST_0:
            return 0;
        case OpcodesRDR3::PUSH_CONST_1:
            return 1;
        case OpcodesRDR3::PUSH_CONST_2:
            return 2;
        case OpcodesRDR3::PUSH_CONST_3:
            return 3;
        case OpcodesRDR3::PUSH_CONST_4:
            return 4;
        case OpcodesRDR3::PUSH_CONST_5:
            return 5;
        case OpcodesRDR3::PUSH_CONST_6:
            return 6;
        case OpcodesRDR3::PUSH_CONST_7:
            return 7;
    ...
    {
        switch (static_cast<OpcodesRDR3>(op))
        {
        case OpcodesRDR3::J:
        case OpcodesRDR3::JZ:
    ...
    uint32_t DisassemblerRDR3::GetJumpTarget(uint32_t pc) const
    {
        OpcodesRDR3 op = static_cast<OpcodesRDR3>(GetU8(pc));

        if (op == OpcodesRDR3::CALL)
            return GetU24(pc + 1);

        return pc + 2 + GetS16(pc + 1) + 1;
    }

    bool DisassemblerRDR3::IsWildcard(uint8_t op) const
    {
        switch (static_cast<OpcodesRDR3>(op))
        {
        case OpcodesRDR3::CALL:
    ...
    bool DisassemblerRDR3::IsXrefToPc(uint32_t pc, uint32_t targetPc) const
    {
        if (m_Code[pc] == static_cast<uint8_t>(OpcodesRDR3::CALL) || m_Code[pc] == static_cast<uint8_t>(OpcodesRDR3::PUSH_CONST_U24)) // check for function pointers
        {
    ...
    std::string DisassemblerRDR3::MakePattern(uint32_t start, int len) const
    {
        ...
        while (i < len && (start + i) < static_cast<int>(m_Code.size()))
        {
            ...
            if (opcode == static_cast<uint8_t>(OpcodesRDR3::NATIVE))
            {
    ...
    bool DisassemblerRDR3::IsPatternUnique(uint32_t pc, int patternLength) const
    {
        int count = 0;
        ...
        for (int i = 0; i + patternLength <= static_cast<int>(m_Code.size()); ++i)
        {
            ...
            while (j < patternLength)
            {
                ...
                {
                    if (a == static_cast<uint8_t>(OpcodesRDR3::NATIVE) && k > 1)
                        continue;
    ...
    Disassembler::BinarySearchPattern DisassemblerRDR3::MakeStringSearchPatterns(const std::string& value) const
    {
        ...
        auto buildNormal = [&](uint32_t index) {
            ...
            if (index < 0x08)
            {
                p.push_back(static_cast<uint8_t>(OpcodesRDR3::PUSH_CONST_0) + index);
            }
            else if (index < 0x100)
            {
                p.push_back(static_cast<uint8_t>(OpcodesRDR3::PUSH_CONST_U8));
                pushLE(index, 1);
            }
            else if (index < 0x8000)
            {
                p.push_back(static_cast<uint8_t>(OpcodesRDR3::PUSH_CONST_S16));
                pushLE(index, 2);
            }
            else if (index < 0x1000000)
            {
                p.push_back(static_cast<uint8_t>(OpcodesRDR3::PUSH_CONST_U24));
                pushLE(index, 3);
            }
            else
            {
                p.push_back(static_cast<uint8_t>(OpcodesRDR3::PUSH_CONST_U32));
                pushLE(index, 4);
            }

            p.push_back(static_cast<uint8_t>(OpcodesRDR3::STRING));
            return p;
        };
        ...
        auto buildPeephole = [&](uint32_t index) {
            ...
            {
                std::vector<std::optional<uint8_t>> p;
                p.push_back(static_cast<uint8_t>(OpcodesRDR3::PUSH_CONST_U8_U8));
            ...
            {
                std::vector<std::optional<uint8_t>> p;
                p.push_back(static_cast<uint8_t>(OpcodesRDR3::PUSH_CONST_U8_U8_U8));
                p.push_back(std::nullopt); // first U8 (unknown)
    ...
    const char* DisassemblerRDR3::GetInstructionDescription(uint8_t opcode) const
    {
    ...
    void DisassemblerRDR3::BuildFunction(uint32_t pc)
    {
        if (pc >= m_Code.size() || GetU8(pc) != static_cast<uint8_t>(OpcodesRDR3::ENTER))
            return;
        ...
        while (pos < m_Code.size())
        {
            ...
            if (op == static_cast<uint8_t>(OpcodesRDR3::LEAVE))
            {
                ...
                if (nextOp == static_cast<uint8_t>(OpcodesRDR3::ENTER) || next >= m_Code.size())
                {
                    lastLeave = pos;
    ...
    std::string DisassemblerRDR3::DecodeInstructionInternal(int index) const
    {
        ...
        while (*fmt)
        {
            ...
            case 'd': // U24
            {
                ...
                if (op == static_cast<uint8_t>(OpcodesRDR3::CALL)) // Print CALL as hex
                {
