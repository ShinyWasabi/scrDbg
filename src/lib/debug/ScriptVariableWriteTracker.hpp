...
namespace scrDbgLib
{
    ...
    {
    public:
        static void Begin(std::uint32_t hash, std::uint32_t index, bool isGlobal);
        static void AddFieldOffset(std::uint32_t offset);
        static void AddArrayIndex(std::uint32_t index, std::uint32_t size);
        static void Finalize(const char* name, std::uint32_t pc, std::int32_t value, bool isStruct = false);
        static void Break();
